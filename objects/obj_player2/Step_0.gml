...
if (state != states.jump)
	stompAnim = 0
if ((state == states.mach3 or (state == states.grabbed && instance_exists(obj_player1) && obj_player1.state == states.mach3) or state == states.mach2 or state == states.climbwall or state == states.hookshot or state == states.machroll or state == states.tacklecharge or state == states.handstandjump or state == states.machslide) && macheffect == 0)
{
	macheffect = 1
	toomuchalarm1 = 6
...
if (!((state == states.mach3 or (state == states.grabbed && instance_exists(obj_player1) && obj_player1.state == states.mach3) or state == states.mach2 or state == states.climbwall or state == states.hookshot or state == states.machroll or state == states.tacklecharge or state == states.handstandjump or state == states.machslide)))
	macheffect = 0
if (toomuchalarm1 > 0)
{
	toomuchalarm1 -= 1
	if (toomuchalarm1 <= 0 && (state == states.mach3 or (state == states.grabbed && instance_exists(obj_player1) && obj_player1.state == states.mach3) or state == states.climbwall or state == states.hookshot or state == states.mach2 or state == states.tacklecharge or state == states.machslide or state == states.machroll or state == states.handstandjump or (state == states.chainsaw && mach2 >= 100)))
	{
		with (instance_create(x, y, obj_mach3effect))
		{
			playerid = other.object_index
...
if (state == states.gottreasure or sprite_index == spr_knightpepstart or sprite_index == spr_knightpepthunder or state == states.keyget or state == states.door or state == states.ejected or state == states.victory or state == states.comingoutdoor or state == states.gameover)
	...
	cutscene = 0
if ((place_meeting(x, y, obj_door) or place_meeting(x, y, obj_dresser) or place_meeting(x, y, obj_snick) or place_meeting(x, y, obj_keydoor) or (place_meeting(x, y, obj_exitgate) && global.panic == 1)) && (!instance_exists(obj_uparrow)) && scr_solid(x, (y + 1)) && state == states.normal && obj_player1.spotlight == 0)
{
