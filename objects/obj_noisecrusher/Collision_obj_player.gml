if (invframes == 0)
{
	if (state != states.grabbed && obj_player.state != states.boxxedpep && obj_player.state != states.bump)
	{
		with (obj_player)
		{
			if (y > other.y && other.vsp > 1 && other.grounded == false && other.state == states.enemy_bounce)
			...
			if ((state == states.mach2 or state == states.mach3 or state == states.grab) && other.grounded == true && other.state == states.enemy_stun)
			{
				global.hit += 1
