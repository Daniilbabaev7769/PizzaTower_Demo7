function scr_player_mach2() {
	...
	if (((!grounded) && place_meeting((x + hsp), y, obj_solid) && (!(place_meeting((x + hsp), y, obj_destructibles))) && (!(place_meeting((x + sign(hsp)), y, obj_slope)))) or (grounded && place_meeting((x + hsp), (y - 64), obj_solid) && (!(place_meeting((x + hsp), y, obj_destructibles))) && (!(place_meeting((x + hsp), y, obj_metalblock))) && place_meeting(x, (y + 1), obj_slope)))
	{
		wallspeed = movespeed
	...
	if (grounded && floor(image_index) == (image_number - 1) && sprite_index == spr_rollgetup)
		sprite_index = spr_mach
	if ((!grounded) && sprite_index != spr_secondjump2 && sprite_index != spr_mach2jump && sprite_index != spr_mach2jump && sprite_index != spr_walljumpstart && sprite_index != spr_walljumpend && sprite_index != spr_playerN_sidewayspin && sprite_index != spr_playerN_sidewayspinend)
		sprite_index = spr_secondjump1
	...
	if ((object_index == obj_player1 && place_meeting(x, y, obj_player2) && obj_player1.hurted == 0 && obj_player2.hurted == 0) or (object_index == obj_player2 && place_meeting(x, y, obj_player1) && obj_player2.hurted == 0 && obj_player1.hurted == 0))
	{
