function scr_player_shoulderbash() {
	...
	if (move != xscale && move != 0)
		state = states.normal;
	if ((floor(image_index) == (image_number - 1) or sprite_index == spr_player_airattack or sprite_index == spr_player_airattackstart) && grounded && (!key_attack) && vsp > 0)
	{
		image_speed = 0.35
		state = states.normal;
		grav = 0.5
	}
	if ((floor(image_index) == (image_number - 1) or sprite_index == spr_player_airattack or sprite_index == spr_player_airattackstart) && grounded && key_attack)
	{
		image_speed = 0.35
		state = states.mach2
		grav = 0.5
	...
	if ((!grounded) && sprite_index == spr_player_attackdash)
	{
		image_index = 0
